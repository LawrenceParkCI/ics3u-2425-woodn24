...
public class NoHsa {
	/**
	Description: From a large list of all 5 letter words one is correct and it is your job to guess it.  You have 6 tries to guess the correct word an if you you dnot you can play again. 
	Date: 2025-01-16
	@author Nathan Wood
	 */
		...
		do {
			...
			do  {
				...
				if ((grammar(guess, wordList)) == true) {
					...
					c.println("That is not a valid word");
					Thread.sleep(1000);
				}
				//Tracks Guess Word
			} while ((wordCorrect(currentWord, guess, win) == false) && (rounds < 6));
			c.println("Game Over");
			c.println("Would you like to play again");
		}  while(c.readLine().equalsIgnoreCase("yes"));

	}
	/**
	 * Checks if the character types a real 5 letter word
